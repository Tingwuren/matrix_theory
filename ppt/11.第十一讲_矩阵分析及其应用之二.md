...
- 先回顾一般函数的幂级数展开与可导性，再用 Jordan 标准形拓宽矩阵函数定义。
- 通过例子说明在不可直接用幂级数的情形（如 $1/z$、$\sqrt{z}$）如何计算矩阵函数。
- 总结矩阵函数的代数性质（加法、乘法），以及矩阵的微分/积分公式，便于后续应用于动态系统和矩阵方程。
...
> 思路: 幂级数展开要求函数在 0 的邻域内各阶导数存在且增长受控，这保证了对标量的收敛，也为矩阵幂级数提供基础。

对于一元函数 $f\left( z\right)  = \frac{1}{z}$ 等,还不能直接用幂级数定义矩阵函数。基于矩阵函数值的 Jordan 标准形算法，可拓宽定义。

> 关键: 只需在矩阵的各特征值处有足够阶导数（至多到 Jordan 块大小减 1），即使函数不能在 0 附近展开也可定义 $f(A)$。

矩阵函数的一般定义

设 ${P}^{-1}{AP} = J = \operatorname{diag}\left( {{J}_{1},\cdots ,{J}_{s}}\right) ,{J}_{i} = {\lambda }_{i}I + {I}^{\left( 1\right) }$ ，其中 ${\lambda }_{i}$ 的代数重数为 ${m}_{i}$。
...
> 实际计算: 先求 $A$ 的 Jordan 形，再对每个 Jordan 块用泰勒展开截断到 ${m}_{i}-1$ 阶，最后相似变换回原坐标。
...
例10: $A = \left\lbrack  \begin{array}{lll} 1 & 1 & 0 \\  0 & 1 & 0 \\  0 & 0 & 2 \end{array}\right\rbrack  , f\left( z\right)  = \sqrt{z}$ ,求 $f\left( A\right)$。

解: $f\left( z\right)  = \sqrt{z},{f}^{\prime }\left( z\right)  = \frac{1}{2\sqrt{z}}$ 。

${\mathrm{J}}_{1} = \left\lbrack  \begin{array}{ll} 1 & 1 \\   & 1 \end{array}\right\rbrack   : \;\mathrm{f}\left( {\mathrm{J}}_{1}\right)  = \mathrm{f}\left( 1\right)  \bullet  \mathrm{I} + {\mathrm{f}}^{\prime }\left( 1\right)  \bullet  {\mathrm{I}}^{\left( 1\right) } = \left\lbrack  \begin{array}{rr} 1 & 1/2 \\   & 1 \end{array}\right\rbrack$

${J}_{2} = \left\lbrack  2\right\rbrack   : \;f\left( {J}_{2}\right)  = f\left( 2\right)  \cdot  I = \left\lbrack  \sqrt{2}\right\rbrack$

> 例子要点: 对 Jordan 块先算标量值和导数，再按公式填入上三角位置；最后将块对角矩阵相似回去即可。
...
$$
f\left( A\right)  = P\left\lbrack  \begin{array}{lll} f\left( {J}_{1}\right) & & \\   &  \ddots  & \\   & & f\left( {J}_{s}\right)  \end{array}\right\rbrack  {P}^{-1}
$$
...
> 提示: 性质(1)(2) 说明矩阵函数在加法、乘法下与标量函数一致；对 Jordan 块的逐块计算保证了与相似变换的协调性。
...
> 直观: 矩阵的导数逐元素定义，后续乘法求导公式与标量链式法则类似，只是注意乘法顺序。

定理8: 设 $A\left( t\right) , B\left( t\right)$ 可导,则有

(1) $\frac{d}{dt}\left\lbrack  {A\left( t\right)  + B\left( t\right) }\right\rbrack   = \frac{d}{dt}A\left( t\right)  + \frac{d}{dt}B\left( t\right)$

(2) ${A}_{m \times  n}, f\left( t\right)$ 可导 $\frac{d}{dt}\left\lbrack  {f\left( t\right) A\left( t\right) }\right\rbrack   = {f}^{\prime }\left( t\right) A\left( t\right)  + f\left( t\right) {A}^{\prime }\left( t\right)$

(3) ${A}_{m \times  n},{B}_{n \times  l} : \frac{d}{dt}\left\lbrack  {A\left( t\right) B\left( t\right) }\right\rbrack   = {A}^{\prime }\left( t\right) B\left( t\right)  + A\left( t\right) {B}^{\prime }\left( t\right)$

证明: (3) 左 $= \frac{d}{dt}{\left( \mathop{\sum }\limits_{k}{a}_{ik}\left( t\right) {b}_{kj}\left( t\right) \right) }_{m \times  l} = {\left( \mathop{\sum }\limits_{k}{a}_{ik}^{\prime }\left( t\right) {b}_{kj}\left( t\right)  + \mathop{\sum }\limits_{k}{a}_{ik}\left( t\right) {b}_{kj}^{\prime }\left( t\right) \right) }_{m \times  l}$
...
(1) $\;\frac{d}{dt}{e}^{tA} = A{e}^{tA} = {e}^{tA}A$ (2) $\;\frac{d}{dt}\cos \left( {tA}\right)  =  - A \cdot  \sin \left( {tA}\right)  =  - \sin \left( {tA}\right)  \cdot  A$ (3) $\;\frac{d}{dt}\sin \left( {tA}\right)  = A \cdot  \cos \left( {tA}\right)  = \cos \left( {tA}\right)  \cdot  A$

证明: $\;\left( 1\right) {e}^{tA} = \mathop{\sum }\limits_{{n = 0}}^{\infty }\frac{1}{n!}{\left( tA\right) }^{n}$ 绝对收敛。
...
> 记忆: 与标量公式同形，因 $A$ 与其幂可交换，左乘右乘结果一致。
...
(2) A 为常数矩阵: ${\int }_{{t}_{0}}^{t}\left\lbrack  {A \cdot  B\left( \tau \right) }\right\rbrack  {d\tau } = A \cdot  \left\lbrack  {{\int }_{{t}_{0}}^{t}B\left( \tau \right) {d\tau }}\right\rbrack$；B 为常数矩阵: ${\int }_{{t}_{0}}^{t}\left\lbrack  {A\left( \tau \right)  \cdot  B}\right\rbrack  {d\tau } = \left\lbrack  {{\int }_{{t}_{0}}^{t}A\left( \tau \right) {d\tau }}\right\rbrack   \cdot  B$

(3) 设 ${a}_{ij}\left( t\right)  \in  C\left\lbrack  {{t}_{0},{t}_{1}}\right\rbrack$ 则: $\frac{d}{dt}{\int }_{{t}_{0}}^{t}\bar{A}\left( \tau \right) {d\tau } = \bar{A}\left( t\right)$

(4) 设 ${a}_{ij}^{\prime }\left( t\right)  \in  C\left\lbrack  {{t}_{0},{t}_{1}}\right\rbrack$ ，则: ${\int }_{{t}_{0}}^{{t}_{1}}{A}^{\prime }\left( \tau \right) {d\tau } = A\left( {t}_{1}\right)  - A\left( {t}_{0}\right)$

> 注意: 乘法下积分仅在常矩阵与可变矩阵相乘时可拆开；(3)(4) 是矩阵版的牛顿-莱布尼茨公式。
...
> 梯度记忆: 二次型 $x^{T}Ax$ 的梯度为 $(A+A^{T})x$，若 $A$ 对称则为 $2Ax$。
